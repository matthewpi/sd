...
const (
	...
	// occurred.
	//
	// The result of calling this will is the same as if [watchdogMessage]
	// wasn't sent in the proper interval.
	//
...
//
// The result of calling this will is the same as if [Watchdog] failed to send
...
// for the application.
//
// If the application is not running under systemd, or the watchdog hasn't been
...
func WatchdogInterval() (d time.Duration, err error) {
	...
	// We got `WATCHDOG_USEC` and `WATCHDOG_PID` matches, return the duration
	// and no error.
	return
}
